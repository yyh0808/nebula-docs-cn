...
在每台机器上下载 TAR 包 [nebula-dashboard-{{ dashboard.release }}.x86_64.tar.gz](https://oss-cdn.nebula-graph.com.cn/nebula-graph-dashboard/{{ dashboard.release }}/nebula-dashboard-{{ dashboard.release }}.x86_64.tar.gz)。
...
## Mac 部署`node-exporter`服务

- 对于 Mac 用户，还可以执行'brew install node_exporter'来安装`node-exporter`服务

- 执行'brew services start node_exporter'启动`node-exporter`服务
